...
/*
Если буду делать второй аккаунт groq, то можно сделать, чтобы они автоматически чередовались
независимо от модели.
Для обоих аккаунтов буду вести счет использованных токенов для обоих моделей
При запросе мы уже знаем, какую модель хотим использовать, поэтому
просто выбираем тот аккаунт, у которого на этой модели меньше использованных токенов
за последнюю минуту (или две минуты).
И не забыть считать сами токены повесив на итоговый результат aiChat обработчик .then
